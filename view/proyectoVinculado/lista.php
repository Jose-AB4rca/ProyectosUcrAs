...
<body>
    ...
    <section class="min-vh-100 h-100 bg-image" style="background-image: url('img/p14.jpg');">
        <div class="container min-vh-100 h-100 bg-light" id="admin-cards">
            <br>
            <h2 class="text-center mb-3">Proyectos vinculados</h2>

            <div class="mt-3 table-responsive text-center">          
            <table class="table display dt-responsive" id="table_id">
                <thead>
                <tr>
                    <th>ID del proyecto - vinculo</th>
                    <th>ID del proyecto vinculado</th>
                    <th>Opción</th>
                </tr>
                </thead>
                <tbody>
                    <?php
                        foreach($this->list as $row){
                            $ob = new ProyectoVinculado();
                            $ob = $row;
                    ?>
                    <tr>
                        <td><?php echo $ob->idProyecto."  |  ".$ob->idProyectoVinculado;?></td>
                        <td><?php echo $ob->proyectoVinculado;?></td>
                        <td>  
                            <a name="del" id="del" onclick="deleteObs('<?php echo $ob->idProyecto;?>','<?php echo $ob->idProyectoVinculado;?>')" class="btn btn-danger m-1"  role="button">eliminar</a>
                        </td>
                    </tr>
                <?php      
                    }
        ...
        <script>
              function deleteObs(idp,ido) {
                const data = [idp,ido];
                if (confirm("Deseas borrar proyecto vinculado: ".concat(data[1]).concat(" ?"))) {
                    location.href = "<?php echo constant('URL').'proyectosVinculados/eliminarProyectoVin/';?>".concat(data);
                }
            }
        </script>
        </script>
