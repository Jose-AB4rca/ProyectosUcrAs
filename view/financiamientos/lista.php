...
<body>
    ...
    <section class="min-vh-100 h-100 bg-image" style="background-image: url('img/p14.jpg');">
        <div class="container min-vh-100 h-100 bg-light" id="admin-cards">
            <br>
            <h2 class="text-center mb-3">Financiamientos del proyecto</h2>
            ...
            <table class="table display dt-responsive nowrap" id="table_id">
                <thead>
                <tr>
                    <th>ID Proyecto / financiamiento</th>
                    <th>Descripción</th>
                    <th>Justificación</th>
                    <th>Tipo</th>
                    <th>Costo</th>
                ...
                <tbody>
                    <?php
                        foreach($this->list as $row){
                            $ob = new Financiamiento();
                            $ob = $row;
                    ?>
                    <tr>
                        <td><?php echo $ob->idProyecto." - ".$ob->idFinanciamiento;?></td>
                        <td><?php echo $ob->descripcion;?></td>
                        <td><?php echo $ob->justificaFi;?></td>
                        <td><?php echo $ob->tipo;?></td>
                        <td><?php echo $ob->costo;?></td>
                        <td>  
                            <a name="editar" id="editar" class="btn btn-warning m-1" href="<?php echo constant('URL').'financiamientos/editar/'.$ob->idProyecto.'?idc='.$ob->idFinanciamiento?>" role="button">editar</a>
                            <a name="del" id="del" onclick="deletefina('<?php echo $ob->idProyecto;?>','<?php echo $ob->idFinanciamiento;?>')" class="btn btn-danger m-1"  role="button">eliminar</a>
                        </td>
                    </tr>
                <?php      
                    }
        ...
        <script>
              function deletefina(idp,ido) {
                const data = [idp,ido];
                if (confirm("Deseas borrar el financiamiento: ".concat(data[1]).concat(" ?"))) {
                    location.href = "<?php echo constant('URL').'financiamientos/borrarfinanciamiento/';?>".concat(data);
                }
            }
        </script>
        </script>
