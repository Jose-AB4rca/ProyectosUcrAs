...
<body>
    ...
    <section class="min-vh-100 h-100 bg-image" style="background-image: url('img/p14.jpg');">
        <div class="container min-vh-100 h-100 bg-light" id="admin-cards">
            <br>
            <h2 class="text-center mb-3">Responsables del proyecto</h2>
            ...
            <table class="table display dt-responsive" id="table_id">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo $ob->idResponsable;?></td>
                        <td class="text-break"><div><?php echo $ob->responsable;?></div></td>
                        <td> 
                        <a name="editar" id="editar" class="btn btn-warning m-1" href="<?php echo constant('URL').'responsables/editar/'.$ob->idProyecto.'?idc='.$ob->idResponsable?>" role="button">editar</a>
                            <a name="del" id="del" onclick="deleteObs('<?php echo $ob->idProyecto;?>','<?php echo $ob->idResponsable;?>')" class="btn btn-danger m-1"  role="button">eliminar</a>
                        </td>
                    </tr>
                <?php      
                    }
        ...
        <script>
              function deleteObs(idp,ido) {
                const data = [idp,ido];
                if (confirm("Deseas borrar responsables: ".concat(data[1]).concat(" ?"))) {
                    location.href = "<?php echo constant('URL').'responsables/borrarResponsable/';?>".concat(data);
                }
            }
        </script>
        </script>
