    ...
    <section class="min-vh-100 h-100 bg-image" style="background-image: url('img/p14.jpg');">
            ...
            <table class="table display dt-responsive" id="table_id">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo $ob->idTematica;?></td>
                        <td class="text-break"><div><?php echo $ob->descripcion;?></div></td>
                        <td> 
                        <a name="editar" id="editar" class="btn btn-warning m-1" href="<?php echo constant('URL').'tematicas/editar/'.$ob->idProyecto.'?idc='.$ob->idTematica?>" role="button">editar</a>
                            <a name="del" id="del" onclick="deleteObs('<?php echo $ob->idProyecto;?>','<?php echo $ob->idTematica;?>')" class="btn btn-danger m-1"  role="button">eliminar</a>
                        </td>
                    </tr>
                <?php      
                    }
        ...
        <script>
              function deleteObs(idp,ido) {
                const data = [idp,ido];
                if (confirm("Deseas borrar temática: ".concat(data[1]).concat(" ?"))) {
                    location.href = "<?php echo constant('URL').'tematicas/borrarTematica/';?>".concat(data);
                }
            }
        </script>
        </script>
