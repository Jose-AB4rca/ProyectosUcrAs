...
<body>
    ...
    <section class="min-vh-100 h-100 bg-image" style="background-image: url('img/p14.jpg');">
        <div class="container min-vh-100 h-100 bg-light" id="admin-cards">
            <br>
            <h2 class="text-center mb-3">Programa adsentos del proyecto</h2>
                ...
                <tbody>
                    <?php
                        foreach($this->list as $row){
                            $ob = new ProgramaAdsento();
                            $ob = $row;
                    ?>
                    <tr>
                        <td><?php echo $ob->idProyecto;?></td>
                        <td><?php echo $ob->idProgramaAdsento;?></td>
                        <td class="text-break"><div><?php echo $ob->programaAdsento;?></div></td>
                        <td>  
                            <a name="editar" id="editar" class="btn btn-warning m-1" href="<?php echo constant('URL').'programasAdsentos/editar/'.$ob->idProyecto.'?idc='.$ob->idProgramaAdsento?>" role="button">editar</a>
                            <a name="del" id="del" onclick="deleteObs('<?php echo $ob->idProyecto;?>','<?php echo $ob->idProgramaAdsento;?>')" class="btn btn-danger m-1"  role="button">eliminar</a>
                        </td>
                    </tr>
                <?php      
                    }
        ...
        <script>
              function deleteObs(idp,ido) {
                const data = [idp,ido];
                if (confirm("Deseas borrar el programa: ".concat(data[1]).concat(" ?"))) {
                    location.href = "<?php echo constant('URL').'programasAdsentos/eliminarProgramaAds/';?>".concat(data);
                }
            }
        </script>
        </script>
