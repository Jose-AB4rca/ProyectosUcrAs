                ...
                <tbody>
                    <?php
                        foreach($this->list as $row){
                            $ob = new InscripcionActividad();
                            $ob = $row;
                    ?>
                    <tr>
                        <td><?php echo $ob->idProyecto." - ".$ob->idInscripcionAc;?></td>
                        <td><?php echo $ob->objetivo;?></td>
                        <td><?php echo $ob->poblacionBeneficiada;?></td>
                        <td><?php echo $ob->cuentaFinanciamientoExt;?></td>
                        <td><?php echo $ob->duracionHoras;?></td>
                        <td>  
                            <a name="editar" id="editar" class="btn btn-warning m-1" href="<?php echo constant('URL').'inscripcionActividades/editar/'.$ob->idProyecto.'?idc='.$ob->idInscripcionAc?>" role="button">editar</a>
                            <a name="del" id="del" onclick="deleteDis('<?php echo $ob->idProyecto;?>','<?php echo $ob->idInscripcionAc;?>')" class="btn btn-danger m-1"  role="button">eliminar</a>
                            <div>
                            <a name="enc" id="enc" class="btn btn-success m-1" href="<?php echo constant('URL').'encargadoActividades/agregar/'.$ob->idInscripcionAc?>" role="button">+ Encargado</a>
                            <a name="enc" id="enc" class="btn btn-success m-1" href="<?php echo constant('URL').'encargadoActividades/listaEspecifica/'.$ob->idInscripcionAc?>" role="button">Ver encargados</a>
                            </div>
